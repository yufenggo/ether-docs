...
## 🔄 重组说明（2026-06-22 · 现行结构以此为准）

> 原 10 块是立项期"从零规划"的拆法。随 6 块冻结、对剩余块重新评估后，**收拢为"6 已冻结模块 + 4 待设计底座"**（下文各"块"描述保留作历史参考，**现行结构以本节为准**）。

**已冻结（6）**：`tenant`(TF-2,原块1)、`auth`(AF-1,块3)、`rbac`(RF-1,块4)、`audit`(AUF-1,块6)、`notification`(NF-1,块10)、`dispatch`(DF-1,横切底座)。

**待设计底座（4）**：
- **B1 安全横切底座**：敏感数据脱敏（日志/审计/响应三处统一）+ 密钥/凭据管理（信封加密）+ 出站调用安全封装（safe-dialer + 3s + 熔断）。〔吸收原块7 密钥/策略、原块8 出站封装〕
- **B2 数据访问底座**（原块5 + 组织模型）：仓储 scope + owner/dept 归属 + 软删/事务 + 数据级权限注入点 + 组织/部门/成员。〔吸收原块8 组织〕
- **B3 平台运营 / 配置中心**（原块2）：系统设置 + 安全策略参数下发 + 平台账号 + 功能开关。〔吸收原块7 策略下发、原块8 备份/可观测〕
- **B4 工程复用 + 治理**（原块9 + helper）：分页/校验/导出/乐观锁 + OpenAPI 协议 + 架构 fitness。

> **原块7 合规**拆解：密钥→B1、安全策略下发→B3、等保核验→B4 治理活动（非代码模块）。
> **设计顺序**：**B1 → B2 / B3 → B4**（B1 最急——脱敏/密钥/出站均被多个已冻结模块引用却悬空）。
