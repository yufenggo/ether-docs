...
> 🔒 **已冻结为 AUF-1（2026-06-22）**：经逻辑/架构/安全三方**三轮审计**修正后封版，汇总成自包含的
> [`specs/audit.md`](../../specs/audit.md)（accepted）。**以 `specs/audit.md` 为权威基线**；
> 本在途稿仅留作设计过程记录，不再更新。要改请新开 `changes/` 变更、评审后再动。

> 在途设计：尚未冻结。标记：✅已定（已与用户拍板） · 🔶建议(待确认) · ⏭️二期/后续 · ⚠️已接受风险。
> 本文件是**块6 审计 一期的自包含设计**，承接 [breakdown.md 块6](../2026-06-platform-foundation/breakdown.md)、落实 [ADR-0006](../../architecture/decisions/0006-audit-unified-stream.md)。
> **本版已纳入逻辑(critic)/架构(architect)/安全(security)三方审计第一轮的修正**（各节 ✅ 标注）。
...
| AU-3 | **写入分级**（✅ 决策1）：判据 = **该动作是否已在某业务 DB 事务边界内**（写 Redis/缓存**不算**）。**在事务内 → 审计同事务强一致**（绝不丢）；**纯判定/读、或写只落 Redis/缓存 → best-effort 独立写 + 告警 + 有界重试**（写失败不回滚业务，解"审计=业务 DoS 单点"）。事件档位对照见 §5 | §5 |
...
**outcome 填写死规则（✅ 修审计 M-1，消歧）**：`outcome` 表示"**该审计所记动作的判定结果**"。
...
- **有界重试与兜底**：无写档的"有界重试"= **进程内即时重试 ≤N 次（不阻塞业务请求线程，超时即放弃），非持久队列**（一期不引入 outbox）；**重试耗尽 → 落本地持久兜底文件 + 高优先告警，绝不静默丢弃**（等保"重要安全事件不丢"底线，✅ 修审计 N1）。
  - **兜底文件防护（✅ 修审计 M-1，防"弱旁路存储"被攻击者偏好抹除）**：兜底文件**与排障 `log` 分开**，必须 **`O_APPEND` 只追加（禁 truncate/seek 重写）+ 权限最小化（仅审计进程 owner 可写、运维只读）+ 纳入 §7 独立信任域 + 不可变备份（Object Lock/WORM）采集范围**；其条目是"**待回灌主表的暂存证据**"——**告警驱动人工/补偿任务回灌 `audit_log`（带原 `event_id` 幂等）**，回表前不视为已满足合规留存；**不得就地删除"清理"**（靠备份+轮转，防当抹证手段）。此路径与 §7"持库特权主体改历史"同属**已接受风险域**，运维补偿（最小权限/堡垒机/操作审计）同样覆盖。
...
### 6.2 跨平面双写：平台侧可靠记录 + 租户侧记录（✅ 决策2 / 修审计 S1/S3/C1/M-1/M-2）
...
- **`correlation_id` 作用域 = 单库内（✅ 修审计 M-1）**：索引 `(correlation_id)` 只在**单库内**聚合有意义（如中心库内 pending+confirmed）。**跨平面两库的同 `correlation_id` 行禁跨库 JOIN**（db-per-tenant 铁律）；**跨平面双写两库行必须填入同一业务请求 id 作 `correlation_id`**（取值一致，否则跨库轨迹断裂）；要看完整跨库轨迹，业务层按 `correlation_id` **分别查两库**，或二期经 audit 自有 outbox 汇聚到统一视图（**二期统一视图物理落点须评估 ADR-0004 数据主权，倾向联邦查询/索引、而非把租户审计行复制进中心库**）。
- **判最终态读语义（✅ 修审计 M-2 + 终审）**：给定 `correlation_id`——**存在 `outcome=success` 的 confirmed 行 = 成功**；**无 confirmed 行**则分两种：① 操作的**同步失败分支**（如 auth.md §4 租户库不可达那一刻）**就地追加一条 `failed` 终态行**（`actor` 为发起平台主体、每阶段至多一行、`ON CONFLICT` 幂等、并发重试不写多条 pending）；② **进程崩溃等"连追加都没发生"的孤儿 pending** → **查询时惰性判定**：`无 confirmed 且 now()-最新行 event_time > 阈值` 则**读侧呈现为 `timed_out`**（纯读计算、**不写新行**，与 AU-5 append-only 最顺，且避免伪造 actor 不明的行）。⚠️ **已接受风险**：一期无后台扫描器，孤儿 pending 靠惰性判定 + §7 进程中断告警人工核，二期 outbox 扫描兜底。
- **不依赖 Dispatch**（✅ 架构 S1）：用 Dispatch 兜底会形成 `audit↔dispatch` 循环依赖 + 初始化死锁 + trace_id 断链，故**不引入**。⏭️ 二期用 **audit 自有 outbox→JetStream**（ADR-0006 §5 传播流，审计专属、非 Dispatch）做最终一致；镜像 `event_id` 主事务内预生成、消费侧 `INSERT ... ON CONFLICT(event_id) DO NOTHING` 幂等（防 at-least-once 重复）。
...
### 6.3 敏感脱敏：白名单 + 值扫描双层（✅ 修审计 A1，致命项）

仅靠"字段名黑名单"会被 `details.extra`、自由文本字段绕过，故**双层**：
1. **白名单准入**：每个 `event_type` 的 `details` 结构在登记表集中定义（§12 待确认2，**冻结前起草首版**）；**未登记字段 → 值替换为 `***UNREGISTERED***` 入库 + 告警**（保留 key 痕迹、可事后补登记，**不静默丢弃**，✅ 修审计 N5）；登记表每个允许字段须标"过值扫描"或"仅结构化非自由文本"，**禁"整段自由文本原样入库"**。
2. **值级模式扫描**：封装层对所有文本值（含 `reason_message`/`target_name`/`details` 各层）正则扫描——疑似 JWT(`eyJ...`)、密钥前缀(`sk-`/`AKIA`)、长 base64/hex、邮箱、手机号——命中即脱敏 + **告警定位漏点**。**只用 Go `regexp`（RE2，线性时间无回溯，天然免疫 ReDoS）+ 单值长度上限**（超长截断+标记，不全量扫），禁第三方 PCRE 回溯引擎（✅ 修审计 N2）；**白名单为主、值扫描为补网**。
3. **绝不入审计**：密码、访问令牌、**激活/重置 token**（含 URL 里的 query，URL 先剥 query）、密钥、DB 连接串、持卡数据。**会话 id 存 `HMAC(sid, 服务端密钥)`**（密钥不入库、复用 §7 哈希链密钥管理；不用无盐 `SHA256`，防低熵 sid 被枚举/彩虹表反推，✅ 修审计 N3）。
...
| **防审计进程中断**（等保硬项,✅ 补缺） | ✅ | 审计写入**健康探针 + 失败告警 + 写入延迟指标**（封装层统一落点）；**一期告警 = 最小信号（ERROR 日志 + 自审计事件，复用 dispatch 同款），非独立告警系统**；"中断"判定阈值（无心跳时长/延迟超阈）实现期定（§12）；审计写失败不得静默 |
...
- **审计读取留痕**（✅ 修审计 D1）：高敏审计查询/导出本身记审计（防"内部人翻看审计无痕"，等保亦视为重要行为）；**"读取留痕"事件自身不再递归记审计**（防无限递归）。
...
> 重置密码等"状态推进"用**追加行**：`pending`/`confirmed` 各一行，共享 `correlation_id`（= `reset_request_id`），**不 UPDATE**（AU-5；细化 tenant.md §9"补 confirmed"为"追加行"）；无 confirmed 且超时则**追加 `failed` 终态行**（§6.2 判最终态）；**`correlation_id` 仅单库内聚合、禁跨库 JOIN**（§6.2）。`session_id` 存 HMAC（AU-6）。应用账号对 `audit_log` 应仅 INSERT/SELECT（统一账号下 DB 级落不了，靠应用层 + 适应度函数，§7）。
...
**⚠️ 已接受风险**：统一账号下"防持库特权主体改历史"未技术落实（运维补偿兜底）；镜像副本一期极端情况可能短暂缺失（权威主记录不丢）。
...
4. `source_ip` 按**可信代理出口 IP 白名单**校验（非单纯跳数 count，防多塞 header 绕过，✅ 修审计 N7）；`source_ip`/`user_agent` 是否按区域脱敏（GDPR，如只存 /24）。
5. 索引细化（JSONB 表达式索引按高频查询补）。
6. event_type 完整枚举清单（随各模块事件补全，集中登记准入）。
7. 备份的独立信任域 / 对象存储 Object Lock 具体方案 + **"应用账号/DBA 删除被拒"的可验证测试证据**（运维 + 块7）。
8. 防审计进程中断的**"中断判定阈值"**（无心跳时长 / 延迟超阈，✅ 修审计 N6）；有界重试的 N 次/超时上限具体值。
