---
title: Dispatch 可靠异步投递底座 · 设计（在途）
status: draft
last_reviewed: 2026-06-21
---

# Dispatch · 可靠异步投递底座（在途设计）

> 🔒 **已冻结为 DF-1（2026-06-21）**：经逻辑/架构/安全三方审计修正后封版，汇总成自包含的
> [`specs/dispatch.md`](../../specs/dispatch.md)（accepted）。**以 `specs/dispatch.md` 为权威基线**；
> 本在途稿仅留作设计过程记录，不再更新。要改请新开 `changes/` 变更、评审后再动。

> 在途设计：尚未冻结。标记：✅已定 · 🔶建议(待确认) · ⏭️后续。
> **本设计只做"底座机制"本身**——不含具体应用功能（邮件、provisioning、审计外发都是它**后续的消费者**）。
> 本版已纳入逻辑/架构/安全三方审计的修正。
...
- 业务池(A) 与 Dispatch 池(B) 取连接前都调 `gate.Check(tenant_code, 读/写意图)`；读 Redis `tstatus:{tenant_code}` 短 TTL 缓存，**fail-closed**(查不到/出错→拒绝)，与 [tenant.md](../../specs/tenant.md) 状态门一致。
...
- **SSRF 出站收口(✅ 底座层强制，不靠 handler 自觉)**：底座**提供并强制 handler 走**统一出站校验封装——协议白名单 + 私有网段黑名单(RFC1918/loopback/link-local) + **连接时再解析校验 IP(防 DNS 重绑定)**，复用 [auth.md](../../specs/auth.md) §5 已定的同一套 SSRF 纪律。**+ 进程级 egress 兜底**：handler 即便绕过封装自起出站，亦在网络层(出站防火墙/代理白名单)被拦——使"强制"从约定升为机制。
