...
> - **TF-2（2026-06-21）**：经 `changes/2026-06-notification` 解冻——§10 边界表"租户通知渠道配置"由🔶范围待定定为 ✅：**渠道配置存租户库、租户可自助配自己的 SMTP（一期）**，详见 [notification.md](notification.md)（NF-1）。
...
| 租户通知渠道配置（飞书 / 钉钉 / 邮件） | 对外集成模块（块10） | per-tenant 渠道配置存租户库、租户自助配 SMTP（一期邮件） | ✅（NF-1，见 [notification.md](notification.md)） |
